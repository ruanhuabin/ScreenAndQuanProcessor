...
from Screen import init, printDict, checkFileValid, extractCompoundNameColumn,\
    ...
    genLSColumn, genRTRangeColumn, genMZDeltaColumn, genMeasuredAreaColumn,extractFormulaColumn,\
    genRTMeasuredColumn
...
#         self.logger.info("base name = " + baseName)
    ...
    def run_thread(self, text_field, outputFilename):
        ...
        extractFormulaColumn(self.inputDataBook, compoundNames, self.outputDataBook, self.logger)
        text_field.insert(INSERT, "[%s]: Extract m/z expected data success\n" %(getCurrTime()))
